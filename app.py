...
# # Function to load the model (uses st.cache_resource for efficiency)
# @st.cache_resource
# def load_model():
#     """Loads the pre-trained DPT model and processor."""
#     try:
#         # st.toast("Loading Deep Learning Model...")
...
# # Function to perform depth prediction
# def predict_depth(image, processor, model):
#     """Processes image and performs depth prediction."""
...
#     # Scaling factor controls the exaggeration of the 3D surface.
#     scaling_factor = 20
#     # Invert the normalized depth: Near objects (low value) are plotted as low Z values.
#     Z_mesh = (1 - normalized_depth) * scaling_factor

#     return output_raw, Z_mesh

# # Function to generate the colored 3D mesh (The hackathon feature)
# def generate_textured_3d_mesh(Z_data, original_image):
#     """Generates a Plotly 3D Surface plot textured with the original image colors."""

#     H, W = Z_data.shape
#     x, y = np.meshgrid(np.arange(W), np.arange(H))

#     # Convert the original image to an RGB numpy array (H x W x 3)
#     img_array = np.array(original_image.convert("RGB"))

#     # Convert RGB array to a grayscale index for 'surfacecolor'
#     # Plotly's go.Surface requires a 2D array for surfacecolor.
#     # We use a weighted average (standard grayscale conversion) of the RGB channels
#     # to serve as the color index, which is then mapped by the custom colorscale.
#     R, G, B = img_array[:, :, 0], img_array[:, :, 1], img_array[:, :, 2]
#     color_index = 0.2989 * R + 0.5870 * G + 0.1140 * B

#     # Generate the custom colorscale where each point maps its index value
#     # to the corresponding original color.
#     # We generate a list of (normalized_index, 'rgb(r,g,b)') tuples.
#     # This is complex in Plotly. For simplicity, we create a dummy colorscale
#     # and rely on the surfacecolor to hold the visual data.

#     # For a visually convincing texture, we create a list of all colors in the image
#     # and use it as the colorscale.

#     # --- The standard Plotly texture trick (simplified) ---
...
#             x=x,
#             y=y,
#             surfacecolor=img_array, # Pass the RGB array directly (Plotly handles this internally)
#             colorscale='YlGnBu', # Use a dummy colorscale for lighting calculation
#             cmin=0, cmax=255,
#             showscale=False
#         )
#     ])
...
#     fig.update_layout(
#         title='3D Digital Twin (Textured)',
#         scene=dict(
#             xaxis_title='Image Width (X)',
#             yaxis_title='Image Height (Y)',
#             zaxis_title='Relative Depth (Z)',
#             aspectratio=dict(x=1, y=H/W, z=0.5), # Adjust aspect ratio
#             aspectmode='manual',
#             camera=dict(
#                 up=dict(x=0, y=0, z=1),
...
# uploaded_file = st.file_uploader("Choose an image (e.g., a room, landscape, or object)", type=["jpg", "jpeg", "png"])

# if uploaded_file is not None:

#     try:
#         # --- FIX FOR PIL.UnidentifiedImageError ---
#         # 1. Read the uploaded file's content (bytes)
#         image_data = uploaded_file.read()
#         # 2. Wrap the bytes in an in-memory binary stream
#         # 3. Open the image using the stream
#         image = Image.open(io.BytesIO(image_data))

#     except Exception as e:
#         st.error(f"Error loading image: {e}. Please ensure the file is a valid JPG, JPEG, or PNG.")
#         st.stop()
...
#         try:
#             output_raw, Z_mesh = predict_depth(image, processor, model)
#         except Exception as e:
...
#         # Normalize and colorize the raw output for visualization
#         formatted_vis = (output_raw * 255 / np.max(output_raw)).astype("uint8")

#         fig_depth, ax_depth = plt.subplots(figsize=(10, 6))
#         ax_depth.imshow(formatted_vis, cmap="magma") # Use 'magma' for high contrast
...
#         try:
#             fig_3d = generate_textured_3d_mesh(Z_mesh, image)
#         except Exception as e:
...
#     st.success("🎉 Processing Complete! Interact with the 3D model above. (Use mouse to rotate and zoom)")
...
def predict_depth(image, processor, model, z_exaggeration_factor):
    ...
    Z_mesh = (1 - normalized_depth) * base_scaling_factor * z_exaggeration_factor
...
def generate_textured_3d_mesh(Z_data, original_image, z_exaggeration_factor):
    ...
    fig = go.Figure(data=[
        go.Mesh3d(
            ...
            lighting=dict(ambient=0.4, diffuse=0.6, specular=0.8, roughness=0.5, fresnel=0.01),
            lightposition=dict(x=1000, y=1000, z=1000), # Light source from a corner
            showscale=False 
        )
    ])
...
st.title("💡 AetherScan: Monocular 2D to 3D Digital Twin Creator (Hackathon Final)")
st.markdown("---")
...
if uploaded_file is not None:
    ...
    with st.spinner("Step 2a: Inferring Depth (Deep Learning Model)..."):
        try:
            ...
            output_raw, Z_mesh = predict_depth(image, processor, model, z_exaggeration_factor)
        except Exception as e:
            st.error(f"Prediction failed: {e}")
            st.stop()
    ...
    with st.spinner("Step 2b: Generating Interactive Textured 3D Mesh..."):
        try:
            ...
            fig_3d = generate_textured_3d_mesh(Z_mesh, image, z_exaggeration_factor)
        except Exception as e:
            st.error(f"3D mesh generation failed: {e}")
            st.stop()
    ...
    st.success("🎉 Processing Complete! Use the **sidebar slider** to adjust the volume of the 3D model.")